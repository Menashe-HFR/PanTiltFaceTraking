...
engine.say("hi, my name is elon mask. I WILL KEEP AN EYE ON YOU")
engine.runAndWait()
...
pi.set_servo_pulsewidth(tilt, 1800)
in_range=10
...
def trackFace(targetX, currentX, targetY, currentY):
    ...
    differenceX = abs(targetX - currentX)
    differenceY = abs(targetY - currentY)
    #-----------------paning------------------------
    if targetX!=500 and differenceX > in_range:
        if targetX>currentX:
            currentX = currentX - (differenceX*increase)
        else:
            currentX = currentX + (differenceX*increase)
        print("<--------- PANING --------->")
    ...
    if targetY!=500 and differenceY > in_range:

        if targetY>currentY:
            currentY = currentY + (differenceY*increase)
        else:
            currentY = currentY - (differenceY*increase)
        print(" \/ \/ \/ TILTING /\ /\ /\ ")
